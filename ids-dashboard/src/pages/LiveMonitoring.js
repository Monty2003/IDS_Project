...
function LiveMonitoring() {
  ...
  const cleanupSocket = () => {
    if (socketRef.current) {
      ...
      socketRef.current.off("live_prediction");
  ...
  const startMonitoring = () => {
    if (isMonitoring || socketRef.current) return;
    ...
    socket.on("connect_error", (error) => {
      ...
      showAlert("danger", "Socket Error", "Could not connect to live stream");
    ...
    socket.on("live_prediction", (data) => {
      const prediction = data?.prediction || "Normal Traffic";
      const attackType = data?.attack_type || "BENIGN";
      ...
      if (prediction === "Attack Detected") {
        ...
        showAlert(
          "danger",
          "Intrusion Alert",
          `${attackType} detected (${confidence}%)`
        );
      } else {
        setStats((prev) => ({
          ...prev,
